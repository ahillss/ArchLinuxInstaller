...
function setup_i3blocks() {
	...
	echo -e '\n#[swap_free]\n#color=#FDEDAA\n#command=awk '"'"'/SwapFree/ {printf("%d\\xcb\\x96\\n", ($2/1000))}'"'"' /proc/meminfo\n#interval=2' >> $HOME/.config/i3blocks/config
	echo -e '\n#[swap_used]\n#color=#FFDDCC\n#command=awk '"'"'/SwapTotal|SwapFree/ {print $2}'"'"' /proc/meminfo | paste -sd'"'"' '"'"' | awk '"'"'{printf "%d\\xcb\\x97\\n",($1-$2)/1000}'"'"'\n#interval=2' >> $HOME/.config/i3blocks/config
